 ...
 * `implicitServiceScopes` - if `true` (default), the single scope `service:<serviceName>:<name>` is required in
   addition to the scope expression specified in `scopes` by the API method caller
